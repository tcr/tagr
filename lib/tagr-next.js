...
var tagr = (function (window, Selection) {
  ...
  DOMTagrElement.prototype._attach = function(parent) {
    this.parent = parent;
    return dommap.set(this._node, this);
  };

  DOMTagrElement.prototype._detach = function() {
    delete this.parent;
    return dommap.remove(this._node);
  };
  ...
  function convertAnchor (win, arg) {
    ...
    if (node.nodeType === 1) {
      if (!(el = dommap.get(node))) {
        return;
      }
      return new ElementAnchor(el, offset);
    } else {
      if (!(el = dommap.get(node.parentNode))) {
        return;
      }
      var i = 0, n = node;
  ...
  function createDOMModule (win) {
    var tagr = createModule(DOMTagrElement, TagrQuery);
    tagr._window = win;
    ...
    tagr['getContext'] = function (node) {
      var el;
      if ((el = dommap.get(node)) == null) {
        domdata.set(node, (el = new TagrElement(node.tagName.toLowerCase(), node, win)));
      }
      return el;
    };
