...
var tagr = (function() {
  ...
  function forEach (obj, fn) {
    return Array.prototype.forEach.call(obj, fn);
  }

  // TODO shim this
  function indexOf (arr, v) {
    return Array.prototype.indexOf.call(arr, v);
  }
  ...
  var spliceObject = function() {
    ...
    obj = arguments[0], i = arguments[1], del = arguments[2], add = 4 <= arguments.length ? slice(arguments, 3) : [];
    if (i < 0 || isNaN(i)) {
      throw 'Invalid index';
    }
    if (del) {
   ...
   * This works naively, or defaults to Sizzle when available.
   */
  ...
  var getQuerySelector = function () {
    ...
    // IE7 and lower
    if (document.documentElement.currentStyle != null) {
  ...
  EventEmitter.prototype['on'] = function() {
    return this.addListener.apply(this, slice(arguments, 0));
  };

  EventEmitter.prototype['once'] = function(type, f) {
    var g = function() {
      f.apply(this, slice(arguments, 0));
  ...
  // tagr.create(...) is a function to create new TagrElements.
  tagr['create'] = function (simple, attrs) {
    ...
    if (arguments.length > 2) {
      el.push.apply(el, slice(arguments, 2));
    }
    return el;
  };
  ...
  tagr['parse'] = function(str) {
    ...
    simple = str[0], attrs = str[1], args = 3 <= str.length ? slice(str, 2) : [];
    return (_ref = tagr.create(simple, attrs)).push.apply(_ref, (function() {
  ...
  // Oblig. domready function.
  tagr['ready'] = domready;

  // Internal. data<->DOM association map.
  tagr._map = new DomMap();

  // Internal. Wrap a DOM node, and create the wrapper if it doesn't exist.
  ...
  // TagrElements map 1:1 with the DOM. They are addressable like arrays by an
  // integer index, though read-only. They support all the functions of array 
  // manipulation and then some. All children are instances of TagrElement or
  // string primitives.
  ...
  TagrElement.prototype["call"] = function (k) {
    return (HTML_DOM_PROPS[k] || k) in this._node
      ? this._node[HTML_DOM_PROPS[k] || k].apply(this._node, slice(arguments, 1))
      : this._props[k].apply(this, slice(arguments, 1));
  };
  ...
  TagrElement.prototype["splice"] = function() {
    ...
    i = arguments[0], del = arguments[1], add = 3 <= arguments.length ? slice(arguments, 2) : [];
    for (j = _i = 0; 0 <= del ? _i < del : _i > del; j = 0 <= del ? ++_i : --_i) {
    ...
    return spliceObject.apply(null, [this, i, del].concat(slice(add)));
  };
  ...
  TagrElement.prototype["insert"] = function (i) {
    this["splice"].apply(this, [i, 0].concat(slice(arguments, 1)));
    return this.length;
  };
  ...
  TagrElement.prototype["removeAll"] = function () {
  ...
  TagrElement.prototype["toHTML"] = function () {
    ...
    for (var k in this._props) {
      if (hasOwnProperty(this._props, k)) {
        str.push(' ' + escapeHTML(k) + '="' + escapeHTML(String(this._props[k])) + '"');
      }
    }
    for (var k in this._persist) {
      if (this._persist[k] && !(k in this._node.attributes)) {
        str.push(' ' + escapeHTML(k) + '="' + escapeHTML(String(this._node[k])) + '"');
      }
    }
    for (var i = 0; i < this._node.attributes.length; i++) {
      var attr = this._node.attributes[i];
      var value = this._node[HTML_DOM_PROPS[attr.name] || attr.name];
      str.push(' ' + escapeHTML(attr.name) + '="' + escapeHTML(String(value)) + '"');
    }
    str.push('>');
  ...
  var TagrQuery = tagr['TagrQuery'] = (function () {
    ...
    TagrQuery.prototype['style'] = mappable(iterkeys(itervalues(function(k, v) {
      var s = getStylesheet();
  ...
  tagr['view'] = {
    ...
    "selection": {
      ...
      "set": function(origin, focus) {
        return Selection.setSelection.apply(Selection, [window].concat(origin.toAnchor(), focus.toAnchor()));
      },
      "remove": function() {
  ...
  // tagr is by default global. Return an instance bound to a
  // different window.
